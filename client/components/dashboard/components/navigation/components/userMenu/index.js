...
const UserMenu = ({open, onClick, setUser, ...props}) => {
  const handleAdminClick = (e) => {
    setUser({role: 'admin', firstName: 'Navin', lastName: 'Ramachandran'})
    onClick()
  }
  const handleNormalClick = (e) => {
    setUser({role: 'normal', firstName: 'James', lastName: 'Whitehall'})
    onClick()
  }
  ...
  return (
    <div className={classNames(styles.userMenu, {[styles.open]: open})}>
      ...
      <UserMenuItem className={styles.userMenu__item} to="/" onClick={handleLogoutClick}>Logout</UserMenuItem>
    </div>
  )
}
